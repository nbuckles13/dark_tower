...
> **AMENDMENT (2026-08-21, fast-fail-guards devloop) — timeout enum moved to the operator lane.** The exit-124/137 → `STATUS=FAIL` mapping above is superseded: both now emit **`STATUS=PRECONDITION_FAILURE`** (the operator lane, exit 2), because a guard timeout is a machine fact (concurrent load / OOM), not a diff defect — indistinguishable-from-a-violation `FAIL` consumed real Gate-2 attempts on clean diffs (2026-08-14 incident). The **REASON tokens are byte-identical** (`guard-timeout-<name>` / `guard-timeout-kill-<name>`) — only the enum changed, so any grep keyed on the token is unaffected. Rationale + triage (incl. the reproduce-on-retry diff-caused discriminator, and the standalone `run-guards.sh` violation/timeout counter split with ladder-mirror exit) live in `docs/runbooks/devloop-validation.md` §3/§6.3 and the fast-fail-guards devloop output. The timeout mechanism itself (this §9) is unchanged; raising/removing the timeout is explicitly NOT the fix.
