...
*Two premises above corrected 2026-08-17 during implementation of story task 4 (R-8).* Corrected in place rather than rewritten, for the reason `docs/user-stories/2026-08-11-story-runner-hardening.md` states about its own corrections: the original sentences remain citable, and the next reader would otherwise re-derive the same dead ends. Both were measured, not reasoned.

**1. "Completeness is `dt-story next` exit 3" is unsafe, and implementing it literally introduces a mutation bug.** `next` is not a read: it mutates on **exactly one path** — selecting an escalated task reopens it (status back to `pending`, `escalation` cleared, file rewritten). That path is reachable **precisely when the completeness check would fail**. So a completeness probe built on `next` is safe only when it passes and destructive when it fails, which is the inverse of what an audit gate must be — it erases the record that a task was escalated at the moment someone is auditing the story, and `/close-story` Phase 3 stages `docs/user-stories/`, so the mutation rides into a commit. It is also non-idempotent: a second `/close-story` sees no escalation, because the first consumed it. **The read-only verb is `list-tasks`**, added for exactly this reason; `/close-story` uses it and asserts `status == "completed"` **positively** (asserting the absence of `pending` would silently pass an escalated task — `Status` has three variants). `docs/TODO.md` had already documented this mechanism against a fixture before this task began.

**2. "Every column it carries is either already in the manifest or belongs there" is false where the containment REVERSES — and that is where deletion becomes destruction.** Measured on `docs/user-stories/2026-05-02-browser-client-join.md`: 65 manifest tasks, of which only 14 carry `prompt`/`specialist`/`env_tests` and 8 carry `deps`, so **51 are bare `id` + `status` stubs**. Its §Devloop Tracking table is the **superset** and the manifest the **skeleton** — the opposite of the relationship this decision assumes. The table also has 60 rows to the manifest's 65 (the runner-appended `audit-remediation-ts` task has no row at all), one row with an empty Devloop Output cell, and a status tally that disagrees with the manifest's. A "mechanical" backfill would therefore require *inventing* slugs and adjudicating between two disagreeing records — hand transcription under ambiguity, which is the very operation this section cites as having produced the original drift.

**That file is the one permanent exception**: its §Devloop Tracking table stays as a frozen historical record. It is `Status: Complete`, nothing writes to it, so no further drift is possible. For every story going forward the new manifest `slug` field means this decision holds with **no** divergence.

**The discriminator to apply before deleting any such artifact** — a test, not a precedent: **is this artifact still written to, AND is the manifest a superset of it?** *Both* must hold. If nothing writes to it, deletion buys no drift protection and only destroys a record. If the manifest is not a superset, deletion loses data.
