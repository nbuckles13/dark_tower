...
2. **The diff-less vector** (new advisory against an UNCHANGED lockfile) is covered by a **weekly scheduled full scan** (`.github/workflows/audit-scheduled.yml`, force-run via `DEVLOOP_AUDIT_FORCE_RUN=1`, suppressions still applied) that surfaces drift within ≤1 week (≪ the §12 14-day MTTR tripwire) and opens a deduplicated `audit-drift` issue, PLUS the **always-run `scripts/audit-suppressions-check.sh`** Layer-3 guard that hard-fails any past-due suppression on *every* run regardless of diff (the calendar is the externality §3 cares about). Neither existed when §3 was authored.
3. §12 14-day MTTR tripwire is unchanged.
...
> **AMENDMENT (2026-08-21, fast-fail-guards devloop) — "runs layer1..7 sequentially" is now conditional on mode.** `layer-all.sh` runs all seven layers sequentially **for UNATTENDED callers only** (CI `GITHUB_ACTIONS`, the story runner / headless devloop sessions `DEVLOOP_HEADLESS`, incl. the story-close authority gate `run-story.sh` which forces run-all with `DEVLOOP_FAIL_FAST=0`) — one pass reports everything broken, which the authority verdict relies on. For **INTERACTIVE** runs it now **fail-fasts**: stops at the first layer exiting non-zero (`rc != 0`), rendering the rest `RESULT=NOT-RUN`, to avoid spending Layers 4-7 (incl. Layer 7's ~10-15 min cluster bring-up) on an already-known red. The mode is config-driven via `DEVLOOP_FAIL_FAST` (resolved by the pure `_common.sh::fail_fast_mode()`); the default derives from unattended detection. "Sequentially" (execution ORDER) is unchanged; what changed is whether the run continues PAST the first red. The honesty invariant holds: `NOT-RUN` never aggregates and never maps to exit 0, so a green run is byte-identical under both modes and fail-fast cannot forge a `GATE2=PASS` (see `docs/runbooks/devloop-validation.md` §3 "Fail-fast vs run-all"). (NB: the earlier attribution that §4 "does NOT stop at the first red layer" appeared in `.claude/skills/devloop/SKILL.md`, not in this ADR — §4 only ever said "sequentially"; SKILL.md now CITES this amendment rather than restating it.)
...
- **Per-toolchain Always-Run audit only on lockfile touch.** **Rejected** (originally): defeats the purpose — the minimatch incident proved transitive vulns surface independent of lockfile diffs in the current branch (they land via Dependabot or sibling devloops merging to main). **SUPERSEDED by the task-#47 amendment (see §3 "Amendment (task #47…)" above):** the scan is now dep-change-gated, legitimate because #47 adds the compensating controls (weekly scheduled full scan + always-run suppressions-check) that did not exist when this alternative was first rejected.
