...
| 4a | Token Refresh Metrics | ❌ Pending | | Add observability for TokenManager: `gc_token_refresh_total` (counter), `gc_token_refresh_duration_seconds` (histogram), `gc_token_refresh_failures_total` (counter by error type). Enables monitoring token acquisition health and debugging AC connectivity issues. |
| 4a | Error Counter Metrics | ❌ Pending | | Add error counter metrics to enable error budget calculations in SLO dashboards: `gc_errors_total` (counter by error_type, endpoint). Track HTTP 4xx/5xx errors, database errors, gRPC errors. Required for `gc-slos.json` error budget panels which currently show no data. Align with ADR-0011 observability framework. |
| 4a | McClient Actor Pattern Evaluation | ❌ Pending | | Current `McClient` uses `Arc<RwLock<HashMap>>` for connection pooling, causing lock contention under load. Evaluate refactoring to actor pattern: single actor owns connections, receives `GetChannel`/`AssignMeeting` messages. Benefits: no lock contention, natural lifecycle management (TTL, failure invalidation, health checks), cleaner concurrency model. Related: channel cache has no TTL and no invalidation on connection failure. |
| 4a | Create Meeting API Endpoint | ❌ Pending | | `POST /api/v1/meetings` endpoint for creating meetings. Required before env-tests can validate authenticated join flow (`GET /api/v1/meetings/{code}`). Currently meetings can only be created via direct DB insertion (init.sql, sqlx::test fixtures). **Deferred env-tests** (removed 2026-02-18 — accepted 404/503 as valid without testing claimed functionality): `test_meeting_join_returns_mc_assignment`, `test_same_meeting_gets_same_mc_assignment`, `test_no_healthy_mcs_returns_503`, `test_join_response_structure_complete`, `test_guest_join_includes_mc_assignment`. **Blockers**: (1) no create endpoint, (2) env-tests only have service tokens — `join_meeting` requires user JWT with UUID `sub` but `test-client` has string `sub`, (3) meeting seed data (org→user→meeting FK chain), (4) MC registration in `meeting_controllers` table, (5) AC meeting-token endpoint requires GC service-to-service auth configured in cluster. **Alternate coverage**: `crates/gc-service/tests/meeting_tests.rs` (sqlx::test with seeded data). |
