...
- WebTransport: connection (join flow, bridge loop, Connection::closed() watch + classify_connection_error, MediaConnectionUpdate handler; async RegisterMeeting trigger R-12 in `register_meeting_with_handlers()`) → `crates/mc-service/src/webtransport/connection.rs`
...
- Prometheus metric wrappers (record_media_policy_push, record_register_meeting, record_mh_notification, record_webtransport_connection, record_jwt_validation, record_session_join, record_token_refresh_metrics, record_participant_leave, record_participant_disconnect, record_display_name_resolution, record_sender_binding_response) → `crates/mc-service/src/observability/metrics.rs`
...
- Client media signalling integration (capability in → directive + assignments out; mute/unmute leaves the directive untouched; seven rejection paths; slot-id echo; client-url redirect guard; edge-handler steering follows placement not Redis order) → `crates/mc-service/tests/media_client_signaling_integration.rs`
