...
- AC metrics -> `crates/ac-service/src/observability/metrics.rs:init_metrics_recorder()`, gauge init `services/key_management_service.rs:init_key_metrics()`, HTTP middleware `middleware/http_metrics.rs`, rate limit config `config.rs`; meeting-token display-name outcome (`ac_meeting_token_display_name_total`) `metrics.rs:record_meeting_display_name_outcome()`, emitted `handlers/internal_tokens.rs:resolve_meeting_display_name()`
- GC metrics -> `crates/gc-service/src/observability/metrics.rs`, HTTP middleware `middleware/http_metrics.rs:normalize_endpoint()`, join wiring `handlers/meetings.rs:{join_meeting,get_guest_token}()` (shared `gc_meeting_join_*` family discriminated by `participant=user|guest`; do NOT fork a `gc_guest_token_*` family), DB metrics `repositories/`; telemetry-proxy + CORS families (`gc_telemetry_ingest_*`, `gc_telemetry_payload_bytes`, `gc_telemetry_rate_limited_total`, `gc_telemetry_pii_attributes_dropped_total`, `gc_cors_preflight_total`) -> `handlers/telemetry.rs`, `services/telemetry_filter.rs`, `middleware/cors_observer.rs`
- MC metrics -> `crates/mc-service/src/observability/metrics.rs`; recording sites: `webtransport/connection.rs:handle_connection()`, `server.rs:accept_loop()`, `grpc/mh_client.rs:register_meeting()`, `grpc/media_coordination.rs` (mc_mh_notifications_received_total); bounded labels `errors.rs:error_type_label()`; display-name resolution (`mc_join_display_name_resolved_total`) `metrics.rs:record_display_name_resolution()` @ `actors/meeting.rs:handle_join()`; participant leave/disconnect (`mc_participant_leaves_total{reason}`, `mc_participant_disconnects_total{cause}`) `metrics.rs:record_participant_leave()`/`record_participant_disconnect()` @ `actors/meeting.rs:handle_disconnect()`
...
- Server/connection/handler (targets: `mc.webtransport`, `.connection`, `.handler`) -> `crates/mc-service/src/webtransport/server.rs`, `connection.rs` (incl. MediaConnectionUpdate), `handler.rs`
...
- Grafana dashboards (per-service overview, errors-overview, SLOs) -> `infra/grafana/dashboards/`, provisioning `infra/grafana/provisioning/`, K8s wiring `infra/grafana/kustomization.yaml`; MC MH Communication row -> `mc-overview.json` (panels: RegisterMeeting RPC Rate, Latency P50/P95/P99); GC Telemetry Ingest + CORS Preflight rows -> `gc-overview.json`; MC display-name/leave/disconnect panels -> `mc-overview.json`; AC display-name panel -> `ac-overview.json`; MH logs + SLO dashboards -> `mh-logs.json`, `mh-slos.json`
- Grafana K8s (configMapGenerator, sidecar, RBAC) -> `infra/kubernetes/observability/grafana/`
- Alert rules (GC, MC, MH) -> `infra/docker/prometheus/rules/{gc,mc,mh}-alerts.yaml`, template -> `_template-service-alerts.yaml`; GC telemetry-proxy alerts (`GCTelemetryProxySilent` page, `GCTelemetryProxyHighRejectionRate` warning) -> `gc-alerts.yaml`; conventions -> `docs/observability/alert-conventions.md`; docs -> `docs/observability/alerts.md`, `dashboards.md`
...
- Per-service deployment + incident response -> `docs/runbooks/` (two per service); GC scenarios 8-9 + join failure triage -> `gc-incident-response.md`, `docs/observability/alerts.md`; GC telemetry-proxy + CORS scenarios 10-13 -> `gc-incident-response.md`, smoke stanzas -> `gc-deployment.md`, OTLP smoke fixture -> `infra/smoke/empty-otlp-metrics.bin`
