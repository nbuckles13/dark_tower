...
- MH metrics (registration, heartbeat, token refresh, gRPC, WebTransport, JWT, RegisterMeeting timeouts R-26) -> `crates/mh-service/src/observability/metrics.rs`; recording sites: `webtransport/server.rs:accept_loop()`, `webtransport/connection.rs:handle_connection()` (incl. provisional-accept timeout arm → `mh_register_meeting_timeouts_total`), `grpc/auth_interceptor.rs:MhAuthService`
