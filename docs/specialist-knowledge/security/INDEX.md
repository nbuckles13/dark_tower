...
- GC `get_guest_token` instrumentation parity audit (ADR-0032 Step 5): adding metric records to a public handler must NOT shift authz — verify each pre-existing predicate (status allowlist, `!allow_guests`, MC assignment, AC request) still gates its return path with metric inserted BEFORE the error return, and `Instant::now()` placed before body validation but no authz check moved earlier → `crates/gc-service/src/handlers/meetings.rs:512-639` (mirrors `join_meeting:338-455`); shared `gc_meeting_join_*` family discriminated by `participant=user|guest` label (cardinality bounded at 2)
- Bounded-set audit pattern for new label values: when a metric grows new `error_type` variants (`guests_disabled`, `bad_request` for guest path), require (a) catalog enumeration updated same-PR with operator-meaningful semantic distinctions, (b) cardinality math under ADR-0011 cap-10, (c) all production call sites use `&'static str` literals not user-controlled values → `docs/observability/metrics/gc-service.md:247-269`
- `actual_type` source-of-truth pinning for `gc_caller_type_rejected_total`: single emission source `claims.service_type.as_deref().unwrap_or("unknown")` at `crates/gc-service/src/grpc/auth_layer.rs:241`. Bounded test ACTUAL_TYPES const must match exactly the AC-issued service-type set + `"unknown"` fallback — fresh test strings would seed unbounded label values cross-service. Rule: do NOT introduce new `actual_type` strings just to drive the rejection path; reuse `"unknown"` or another bounded production value → `crates/gc-service/tests/caller_type_rejected_metrics_integration.rs:34-39`
- Cat B closure byte-equivalence verification heuristic (ADR-0032 §Rollout): for `with_on_refresh`-style closures extracted to `record_*_metrics(&Event)` siblings, verify (a) closure has zero captures from environment, (b) extracted body is byte-identical modulo `event` → `&event` reference, (c) no behavior change at call site. Pure refactor — no security surface — but worth the ~30s diff check to confirm during Step-5-style metric-testability migrations → `crates/gc-service/src/main.rs:124-126` ↔ `crates/gc-service/src/observability/metrics.rs:302-305`
- WRAPPER-CAT-C cluster-test pattern: when full real-recording-site drive needs nontrivial harness (~50 LoC tonic JWKS fixture for `gc_jwt_validations_total`), accept supplementary direct-recorder coverage with adjacency catchers on the bounded enum AS LONG AS (a) header explicitly frames as WRAPPER-CAT-C with rationale, (b) full-fidelity end-to-end coverage exists elsewhere (e.g. `auth_tests.rs` for GC), (c) deferred drive tracked in `docs/TODO.md` §Observability Debt with LoC estimate → `crates/gc-service/tests/jwt_validation_metrics_integration.rs:7-24`
- Test-fixture key derivation: deterministic Ed25519 from `seed: u8` → 32-byte derived seed via formula (`seed_bytes[i] = seed.wrapping_mul(i).wrapping_add(i)`) is the established pattern; PKCS#8 envelope built locally to avoid widening dep graph for test-only signing; `password_hash` placeholders use literal `'hashed'` not real bcrypt/argon2 → `crates/gc-service/tests/common/jwt_fixtures.rs`
