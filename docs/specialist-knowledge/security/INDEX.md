...
- **Guard matcher families — pick before adding vocabulary.** Word-boundary `\b…\b` (the `rust_pii.rs` / `metric_labels.rs` idiom) **cannot see inside camelCase or compound names**: `\btoken\b` matches neither `userToken` nor `user_token`, and `\bkid\b` does not match `sender_kid`. Use segment equality — `crates/dt-guard/src/ts_retained_credentials.rs:198::segments()` (`pub`, splits on `_`/non-word/camelCase; tests `:972`, `:983`) — whenever a vocabulary entry must cover compound spellings. Rediscovered independently in the 2026-08 media-flow debate; at a third consumer, promote `segments()` into `crate::common::` per the Pattern-B rule that moved `pii_vocabulary`.
