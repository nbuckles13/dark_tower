...
- Guard pipeline as single Rust binary `dt-guard` (DFA-vs-backtracking ReDoS-class elimination via `regex` crate as load-bearing security property; `fancy-regex` explicitly rejected to preserve linear-time guarantee; `serde_yaml` typed-deserialization with `#[serde(deny_unknown_fields)]` for compile-time YAML schema validation; SHA256-pin + image-digest-pin contract preserved for D-slate vendor binaries `promtool` + `grafana/grafana@sha256:…`) → ADR-0034 | `resolve_cited_path` canonical containment gate (component-wise `Path::starts_with`, real-filesystem OSError-graceful via `std::fs::canonicalize` `.ok()?` on dangling symlinks — no CPython-mock fragility) → `crates/dt-guard/src/common/path_safety.rs` (lands per ADR-0034 §5) | 7-case symlink-escape veto-blocking test set (containment-positive, traversal-escape, absolute-path-escape, symlink-escape, symlink-inside, dangling-symlink/OSError-graceful, equality-with-root; `#[cfg(unix)]` replaces `os.supports_symlinks` skip-guard) → `crates/dt-guard/tests/doc_cite_resolve.rs` | Structural duplication impossible by construction via workspace `clippy.toml disallowed_methods = [{ path = "regex::Regex::new", reason = "..." }]` + `pub static Lazy<Regex>` in canonical module — collapses the meta-guard layer the predecessor debate would have required into ~5 LoC → workspace `clippy.toml`

- Client credential-lifetime enforcement (retention-gated; segment-matched field vocabulary partitioned from `common::pii_vocabulary::PII_TOKENS_CATEGORY_A`; full-tree `packages/**`; NO bypass marker by design) → `crates/dt-guard/src/ts_retained_credentials.rs`, wrapper `scripts/guards/simple/ts/no-retained-credentials.sh` | Semantic lens (two gates: retention + transmission-while-token-held) → `scripts/guards/semantic/checks.md` §Client Credential Lifetime | Token-based join (no password retained past the exchange) → `packages/sdk-core/src/session/events.ts::TokenCredentials`, `packages/web-app/src/lib/types.ts::AuthSession`
