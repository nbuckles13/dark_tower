...
- Subscriber registry (`LocalSubscribers`, composite `SubscriberKey{meeting,sender}` unconstructible without a `MeetingKey`, ArcSwap, read per frame) + `SenderBindings` — the participant→`sender_id` map whose WRITER DOES NOT EXIST: no contract carries the association, every client-asserted route is a cross-participant injection primitive, so the forward path fails closed and loudly (`docs/TODO.md` §Media Path Obligations, first entry) → `crates/mh-service/src/session/mod.rs`
