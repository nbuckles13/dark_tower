...
- MC/MH per-instance ConfigMaps → `infra/services/{mc,mh}-service/{mc,mh}-{0,1}-configmap.yaml` (per-instance values only; shared values in `*-service-config`)
...
- setup/teardown (ADR-0030) -> `infra/kind/scripts/{setup,teardown}.sh`; ConfigMap patching -> `setup.sh:deploy_{mc,mh}_service()`
- Helper commands -> `crates/devloop-helper/src/commands.rs`; dev-cluster display -> `infra/devloop/dev-cluster:display_cluster_info()`
- devloop.sh infra health check -> `infra/devloop/devloop.sh`; Layer 8 env-tests -> `.claude/skills/devloop/SKILL.md`
