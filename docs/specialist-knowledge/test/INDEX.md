...
- Join flow + WebTransport tests (T1-T15 MH assignment + media_servers + bridge + RegisterMeeting trigger T12/T13 + multi-MH T14 + grpc_endpoint=None skip T15 + retry/backoff `tests::test_register_*`) -> `crates/mc-service/tests/join_tests.rs`, `src/webtransport/connection.rs:tests`
