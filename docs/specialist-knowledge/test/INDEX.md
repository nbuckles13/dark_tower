...
- Metric testability + `MetricAssertion` patterns (per-thread recorder, counter-idempotent-on-repeat-read, histogram-drain-on-read, two-fixed-point timing with ONE snapshot, partial-label `assert_delta(0)` subset-match, current_thread flavor load-bearing for `tokio::spawn`; `assert_unobserved` symmetric API across counter/gauge/histogram with `ensure_no_kind_mismatch` hardening — hard form distinct from soft `assert_delta(0)`/`assert_observation_count(0)` by panic-on-cross-kind-regression; closes ADR-0032 §F4; symmetry table + drain-on-read trap proof in module doc; per-cluster `assert_only_<combo>` adjacency-helper pattern catches label-swap-bug regressions; production-truth multi-emission variant asserts ALL real emissions on chained-call paths; reviewer scope-fidelity discipline = grep plan-stage commitments against landed code, don't let API-side polish substitute for integration-side fidelity at close-out) -> `docs/decisions/adr-0032-metric-testability.md`, `crates/common/src/observability/testing.rs` (§"Delta semantics", §"Unobserved semantics", per-kind `assert_unobserved` impls, proof-of-trap + kind-mismatch tests), `crates/ac-service/tests/audit_log_failures_integration.rs:106-213` (`assert_only_event_type` + multi-emission), `crates/ac-service/tests/credential_ops_metrics_integration.rs:54-71` (`assert_only_cell`), `docs/devloop-outputs/2026-04-26-adr-0032-step-4-ac-metric-test-backfill/main.md` (iter-2 closure)
- Orphan-recording-site disposition pattern (ADR-0032 Step 5; metric whose recording fn has zero production callers — driving it at the real seam proves wiring, not behavior, and is misleading. Reclassify to wrapper-Cat-C with a *distinct* canonical comment block calling out the orphan classification + handler-integration gap, separate from the no-business-error-branch wrapper-Cat-C variant. Index both variants in the cluster-file header docstring so readers don't infer which class of un-drivability they're looking at. Track in TODO §Observability Debt as a separate entry from fault-injection-harness debt — fixes are different: caller wiring vs harness build. Heuristic: any time a wrapper-Cat-C demotion is being discussed, do the wider grep — `git grep -- 'crates/'` outside the repo's own file — to confirm whether the surrounding *struct or trait* is orphan, not just the one fn flagged. Initial F1 finding scoped narrowly to one fn missed that the entire `ParticipantsRepository` was orphan; wider grep extended the demotion to all 3 ops) -> `crates/gc-service/tests/db_metrics_integration.rs:9-39,216-292` (dual-canonical-block indexing + orphan-fn comment block), `docs/TODO.md` §Observability Debt "Orphan recording-site audit" entry, `docs/devloop-outputs/2026-04-27-adr-0032-step-5-gc-metric-test-backfill/main.md`
- Gauge 4-cell adjacency matrix as exemplary reference (`assert_value(0.0)` vs `assert_unobserved` distinction is load-bearing — the former asserts the metric IS observed at zero (zero-fill writer wrote `set(0.0)` explicitly), the latter asserts the metric was NEVER observed (no emission at all). Conflating them masks always-emit refactors. The 4 cells are: full happy / partial counts → zero-fill via `assert_value(0.0)` / empty counts → all zero / caller short-circuits → `assert_unobserved` across full label space. Cell 4 distinguishes "always-emit refactor regression" from "zero-fill correctness regression" — orthogonal coverage axes, both needed.) -> `crates/gc-service/tests/registered_controllers_metrics_integration.rs` (canonical 4-cell shape; reuse for any gauge with explicit zero-fill semantics)
