...
## Feature-gated test crates are CI-safe by design — verify this before assessing impact (env-tests-fix, 2026-02-18)

The env-tests crate uses Cargo feature gates (`smoke`, `flows`, `observability`, `resilience`) with no default features. This means `cargo test --workspace` compiles the crate but runs zero tests, making env-tests changes inherently CI-safe. When reviewing env-tests modifications (converting skips to panics, changing URLs, removing tests), the first operations question should be "does CI enable any features?" — and the answer in this project is no (see `.github/workflows/ci.yml` line 67: `cargo test --workspace --verbose` with no `--features`). This is a 30-second check that eliminates the entire class of "will this break CI?" concerns and lets the review focus on whether the tests are correct for their actual execution environment (Kind cluster with port-forwards).

## Per-prefix dashboard ConfigMaps with k8s-sidecar avoid the 262144-byte annotation limit (env-tests-fix iter 2, 2026-02-18)

When Grafana dashboards exceed ~250KB total, a single monolithic `kubectl create configmap --from-file=dashboards/` will hit the 262144-byte client-side annotation limit on `kubectl apply`. The fix has three parts: (1) split dashboards into per-prefix ConfigMaps (`grafana-dashboards-ac`, `grafana-dashboards-gc`, etc.) using filename prefix grouping, (2) use `--server-side` apply to bypass the annotation limit entirely, and (3) replace the direct ConfigMap volume mount with an `emptyDir` volume populated by a `kiwigrid/k8s-sidecar` initContainer that discovers ConfigMaps by label (`grafana_dashboard=1`). The sidecar approach also enables hot-reload of dashboards in future (by switching `METHOD` from `LIST` to `WATCH`). The non-obvious step: the legacy monolithic `grafana-dashboards` ConfigMap must be explicitly deleted (`--ignore-not-found`) before creating the new per-prefix ones, because the Grafana provisioner would otherwise load duplicate dashboards from both the old and new sources.
