...
## Test fixture reviews surface infrastructure gaps as side-effects (env-tests-fix, 2026-02-18)

While reviewing env-tests URL corrections, cross-referencing against Kind cluster configs (NetworkPolicies, configmaps, service definitions) surfaced a pre-existing GC→MC NetworkPolicy gap: GC's `network-policy.yaml` has no egress rule for port 50052 (MC's gRPC port), meaning GC cannot initiate connections to MC even though MC→GC ingress on port 50051 is allowed. This was correctly scoped out of the env-tests fix task but demonstrates that test fixture reviews — where you verify "do these test URLs match what the infrastructure actually exposes?" — are a good opportunity to audit the infrastructure configs you're cross-referencing. The env-tests themselves will eventually need MC-GC integration tests that depend on this egress rule existing.
