...
## Dashboard panels for unwired metrics create silent "No data" confusion (gc-registered-mc-metrics, 2026-02-16)

When metrics recording functions exist in `metrics.rs` but are marked `#[allow(dead_code)]` (never called from production code), adding Grafana dashboard panels for them creates a "No data" display that is worse than having no panel. During incidents, on-call sees "No data" and cannot distinguish between: (1) metric scrape is broken, (2) service is down, or (3) the recording function was simply never wired to a call site. The failure mode is wasted debugging time at the worst possible moment. In this project, 10 AC metrics (`ac_db_*`, `ac_bcrypt_*`, `ac_rate_limit_*`, `ac_audit_log_*`, `ac_admin_operations_total`, `ac_token_validations_total`, `ac_signing_key_age_days`, `ac_active_signing_keys`, `ac_key_rotation_last_success_timestamp`) had recording functions defined but never called. The fix: if placeholder panels are added anyway (to satisfy a metrics-completeness guard), their descriptions must include "Pending instrumentation" or similar text so on-call knows "No data" is expected, not broken.
