...
| `scripts/lang/_get_base_ref.sh` (inline TODO comment near ci-pr branch, pointing at docs/TODO.md entry) | Mine | — |
| `docs/TODO.md` (4 entries appended: alert-aggregation conventions [Observability Debt, relocated from misplaced /work/TODO.md], `_get_base_ref.sh` CI-PR tip-SHA [Polyglot Pipeline Follow-ups], `_get_base_ref.test.sh` rc=0 assert [Polyglot Pipeline Follow-ups], SHA-pin GH Actions [Supply Chain]) | Mine | — |
...
3. **`scripts/lang/_get_base_ref.sh` + `docs/TODO.md`** (Gate 3 F1 cleanup) — code-reviewer flagged the per-wrapper `TODO(post-#36)` comment as misplaced (the bug is in `_get_base_ref.sh`, not in consumers) and duplicated 4× across the wrappers. Moved the TODO marker to its proper home: one comment line in `_get_base_ref.sh` near the `ci-pr` branch (line ~85) referencing a structured entry in `docs/TODO.md` under "## Polyglot Pipeline Follow-ups (ADR-0033 Wave 1 #1)". The wrappers no longer carry the comment. (Original landing was in misplaced `/work/TODO.md`; relocated to canonical `docs/TODO.md` per SKILL Step 9 in the post-commit follow-up below.)
...
- **Polyglot Pipeline Follow-ups (ADR-0033 Wave 1 #1)** — `_get_base_ref.sh` CI-PR tip-SHA vs merge-base-SHA divergence (P3 perf nit; conservative over-affected). Inline anchor at `scripts/lang/_get_base_ref.sh:86-90`. See `docs/TODO.md` § Polyglot Pipeline Follow-ups.
- **Polyglot Pipeline Follow-ups (ADR-0033 Wave 1 #1)** — `_get_base_ref.test.sh` should explicitly assert rc=0-iff-non-empty-stdout to defend the empty-guard-drop contract. Test-reviewer Gate-1 follow-up; bundle with the tip-SHA fix above. See `docs/TODO.md` § Polyglot Pipeline Follow-ups.
- **Supply Chain** — SHA-pin all third-party GitHub Actions in `.github/workflows/ci.yml` (currently 7 tag-pinned, one channel-pinned). Security Gate-1 follow-up; P2. See `docs/TODO.md` § Supply Chain.

**Pre-existing layer failures noted at Gate 2** (not new tech debt, captured here for the devloop record only):

- `proto-gen:lint` — `nx affected -t lint` exits 1 due to pre-existing buf-lint findings in `proto/internal.proto` and `proto/signaling.proto` (file-layout, package-naming, RPC-naming conventions). Orthogonal to this task; tracked under R-61 (Track 2 #29 lint.ignore scaffolding will mute during cleanup window; #30/#31 do the rename sweep).
- `cargo audit` RUSTSEC-2023-0071 — `rsa 0.9.10` Marvin Attack via `sqlx-mysql`; "No fixed upgrade available." Pre-existing on `main`, not introduced by this diff.
