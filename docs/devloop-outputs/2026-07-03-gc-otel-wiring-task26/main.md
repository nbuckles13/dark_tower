...
- **Pre-existing branch reds (NOT task-26, out of scope, surfaced to user)**: Layer 1 `nx-typecheck-failed` = client SDK packages fail `tsc` (missing proto→TS codegen R-6 / incomplete client tasks; zero TS in this diff); `validate-todo-tracking` violation is in task #14's devloop doc (`2026-06-29-…task14/main.md:231`). A fully-green whole-branch pipeline is not currently achievable due to incomplete client work — independent of this devloop.
