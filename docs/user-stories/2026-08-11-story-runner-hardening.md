...
- *A sixth defect, unreachable as written* — a `sort | head` pipeline under `pipefail` can kill the runner via SIGPIPE. It cannot fire today: input is scoped by `-newer "$start_marker"` and the marker is re-stamped before every read, so the window never spans more than one task's execution, and serial execution bounds what it can contain. Both bounds are emergent from resume-detection code that was not written to provide them.
