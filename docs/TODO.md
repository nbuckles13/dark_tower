...
- [ ] **Endpoint scheme validation duplication (GC/MH)**: `crates/mh-service/src/grpc/mh_service.rs:133-135` and `crates/gc-service/src/grpc/mh_service.rs:110-112` both inline `starts_with("http://") && starts_with("https://") && starts_with("grpc://")`. GC already has a `validate_endpoint()` helper; `crates/gc-service/src/grpc/mc_service.rs:162` does a similar check without `grpc://`. Could extract to `common::validation::validate_grpc_endpoint()` if a fourth occurrence appears. Low priority (3 call sites).
- [ ] **Field length limit constants not centralized (GC/MH)**: `crates/mh-service/src/grpc/mh_service.rs` defines `MAX_ID_LENGTH = 256` and `MAX_ENDPOINT_LENGTH = 2048`. `crates/gc-service/src/grpc/mh_service.rs` defines its own `MAX_ENDPOINT_LENGTH = 255`. Different values for similar purposes. Consider extracting to `common::validation::{MAX_ID_LENGTH, MAX_ENDPOINT_LENGTH}` if more services adopt these limits. Low priority (2 services).
