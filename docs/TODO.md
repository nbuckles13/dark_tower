...
- [ ] **`add_auth` helper duplication (3 call sites)**: `crates/mc-service/src/grpc/gc_client.rs:GcClient::add_auth`, `crates/mc-service/src/grpc/mh_client.rs:MhClient::add_auth`, `crates/mh-service/src/grpc/gc_client.rs:GcClient::add_auth` — all ~10 lines identical (create Request, get token from TokenReceiver, format Bearer header, insert metadata, map parse error). Extract crate-local free function in MC's `grpc/mod.rs` first; extract to `common` when a third crate needs it.
