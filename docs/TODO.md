...
- [ ] **TestKeypair/build_pkcs8_from_seed duplication (8 locations)**: Identical Ed25519 test keypair helpers duplicated in: `crates/gc-service/src/grpc/auth_layer.rs` (tests), `crates/gc-service/tests/auth_tests.rs`, `crates/gc-service/tests/meeting_create_tests.rs`, `crates/gc-service/tests/meeting_tests.rs`, `crates/ac-test-utils/src/crypto_fixtures.rs`, `crates/mc-service/src/auth/mod.rs` (tests), `crates/mc-service/src/grpc/auth_interceptor.rs` (tests), and `crates/mh-service/src/grpc/auth_interceptor.rs` (tests). Should consolidate into a shared test-utils crate (e.g., extend `ac-test-utils` or create `common-test-utils`).
...
- [ ] **`mock_token_receiver` test helper duplication (3 copies in MC)**: `crates/mc-service/src/grpc/gc_client.rs` (tests), `crates/mc-service/src/grpc/mh_client.rs` (tests), `crates/mc-service/tests/gc_integration.rs` — identical `OnceLock<watch::Sender<SecretString>>` pattern. Candidate for extraction to `mc-test-utils`.
- [ ] **GrpcAuthLayer extraction (MC/MH)**: Both MC and MH now have async JWKS-based auth layers (`McAuthLayer`/`McAuthService` and `MhAuthLayer`/`MhAuthService`) that are structurally near-identical tower Layer/Service implementations. Differences: MC skips scope check at layer level, MH checks `service.write.mh`; tracing targets differ. Extract to `common::grpc::GrpcAuthLayer<F>` parameterized by scope-check closure. Trigger: third service needing the pattern. Locations: `crates/mc-service/src/grpc/auth_interceptor.rs:McAuthLayer`, `crates/mh-service/src/grpc/auth_interceptor.rs:MhAuthLayer`.
- [ ] **Dead McAuthInterceptor (MC)**: `McAuthInterceptor` is no longer applied in production -- replaced by `McAuthLayer` at the tonic server level in `main.rs`. Only used in tests. Remove or gate behind `#[cfg(test)]`. Location: `crates/mc-service/src/grpc/auth_interceptor.rs:McAuthInterceptor`.
- [ ] **NoopService test helper duplication (MC/MH)**: Identical `NoopService` struct (tower Service returning empty OK response) in MC `auth_interceptor.rs` tests and MH `auth_interceptor.rs` tests. Extract alongside TestKeypair to shared test-utils.
