...
- [ ] **GC integration test fixture duplication**: `crates/gc-service/tests/participant_tests.rs`, `meeting_create_tests.rs`, `meeting_tests.rs` each re-implement org/user/meeting INSERT helpers, `TestClaims`/`TestUserClaims` structs, `TestKeypair`, `build_pkcs8_from_seed()`, and `get_test_metrics_handle()`. Within `participant_tests.rs`, `create_test_fixtures_with_status` and `create_test_fixtures_with_max` duplicate the same org/user/meeting INSERT logic differing only in parameterized fields. Consider a shared `tests/common/mod.rs` or `gc-test-utils` module.
