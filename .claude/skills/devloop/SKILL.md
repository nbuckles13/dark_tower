...
1. PLANNING: Draft your approach, use SendMessage to share your plan with reviewers for input. If your plan includes cross-boundary edits (files owned by another specialist), self-classify each per §Cross-Boundary Edits (Mechanical / Minor-judgment / Domain-judgment) in the plan. Before sharing: try restating the problem in mechanism-language rather than instance-language (if the task says "fix the X files," the mechanism might be "consolidate Y constants"). If the restated mechanism would forbid using the task's own nouns and produces a wider class with more same-owner siblings than the task names, surface that to reviewers — the task's framing may be narrower than the problem actually is.
   ...
   **Complete the invariant — never ship it partial.** If your change establishes a *consistency rule* (a formatter applied one way, a config-not-hardcode pattern, an error taxonomy, a metric-label convention) and there are instances of that rule **outside your stated scope** — including in another domain's files — the default is to **complete it in this devloop**: edit those files too and pull the owner into planning + review (§Cross-Boundary Edits — any file is editable in-loop with the owner in the loop). Do **not** stay in your lane and leave a rule "everywhere except X" — a half-applied invariant is a landmine, worse than not starting. Deferring because "that file is another domain's / a GSA" is the exact stale instinct to avoid; the owner is pulled in, not the work deferred. Only if completing the invariant genuinely will not fit one session, **hold the whole behavior change** and keep the consistent status quo (do not ship the partial), and surface that to the Lead as a sizing call — a consistent-but-less-convenient state beats an inconsistent-but-more-convenient one.

   **Resolving completion-level choices (Lead).** When the plan is settled except for *how far to complete an invariant* — a carve-out versus the fully-uniform version — that is a design preference, not a fork for the operator. **Default to the most complete option that (a) fits one devloop session and (b) crosses no hard safety line** (a genuine wire-*semantic* change, a security veto, a destructive or irreversible operation). "More complete" is the default because the point is to not leave the invariant partial.
...
**The `DEVLOOP_FMT_APPLY=1` prefix is the fmt auto-apply opt-in (ADR-0037 §D7).** Under the check-default inversion, Layer 2's fmt lanes are read-only `--check` unless a caller explicitly opts in. An INTERACTIVE devloop opts in here: `cargo fmt`/`buf format` are deterministic, semantically-empty transforms, so auto-applying them locally masks nothing and saves the implementer a round-trip (a Layer-2 `FMT_APPLIED=<files>` line reports what was rewritten; the layer still emits `STATUS=OK`, per ADR-0033). The single-home lane decision lives in `fmt_mode()` (`scripts/lang/_common.sh`) — do NOT add `if $CI` branches here. This opt-in is deliberately absent from the two tree-attesting callers: the run-story authority gate and `.githooks/pre-commit` force `DEVLOOP_FMT_CHECK_ONLY=1` (which beats this opt-in in `fmt_mode()`'s precedence — fail-closed), because an attestation must not mutate the tree it certifies. In `GITHUB_ACTIONS`/`CI` this prefix is a no-op (CI beats the opt-in): CI stays check-only and fails loud on drift.
