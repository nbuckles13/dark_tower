...
7. **Partial-invariant check — @dry-reviewer carries this charge.** Does the plan establish a *consistency rule* (a formatter applied one way, a config-not-hardcode pattern, an error taxonomy, a metric-label convention), and does it cover **every** instance of that rule — or is it a lane-cut that will ship the rule "everywhere except X"? Enumerate the instances (the same exhaustive walk as a duplication finding). If the plan covers only some, **challenge it**: the missing instances are in scope — pull their owner into planning + review (cross-boundary edits are done in-loop, not deferred) — or, if completing the rule genuinely will not fit one session, the whole behavior change is *held* (consistent status quo), never shipped partial. A half-applied invariant is a scope defect, cheapest to catch here at planning; a shipped one is worse than an unstarted one. Any reviewer may raise it, but it is @dry-reviewer's explicit responsibility.
...
**A shipped partial invariant is a finding → ESCALATE (any reviewer).** If the diff establishes a consistency rule but applies it to only some of its instances — a rule "everywhere except X" — that is a defect of the same class as masking or SSoT-drift, not an acceptable scope boundary. Raise it: the missing instances are completed in-loop (pull the owner in), or the whole behavior change is held for a complete follow-up; a deferral that leaves the codebase inconsistent is refused. This is the Gate-3 backstop for the @dry-reviewer Gate-1 partial-invariant check above, and it catches light-tier tasks that skipped Gate-1. **But a choice of *how far* to complete an invariant (a carve-out vs the fully-uniform version) is a design preference, not an escalation** — don't push the Lead to escalate it; the Lead defaults to the most complete option that fits one session and crosses no hard safety line (and headless, resolves it without blocking). Escalation is for a partial that ships *unreasoned*, not for picking between reasoned completion levels.
