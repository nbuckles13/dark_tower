...
**Task granularity** — size each task as the largest *independently-verifiable behavioral increment* that completes in one devloop session, **not the smallest seam**. Each task still has one implementing specialist.
...
- **`--tier` sets the Gate-1 planning-round tier.** The lead sets each task's tier here, at decomposition time (where the author already reasons about contract surfaces), from this **mechanical rule** — **`full`** when the task **delivers `proto` / touches a wire contract or public API**, **introduces or moves an architectural seam**, **spans >1 service/specialist**, **is author-flagged ambiguous**, **OR touches a Guarded Shared Area or an auth / crypto / secrets / tenancy-isolation surface**; **`light`** otherwise, with a one-line `--tier-reason`. The default errs toward `full` wherever a contract signal exists.
  - **Security extension to the full-tier triggers**: the final criterion — GSA / auth / crypto / secrets / tenancy-isolation — is a deliberate security-motivated addition to the triggers above. Plan-stage review is worth materially more than diff-stage review for crypto/secrets work, and this criterion is checkable from the task's own file list, so it is always cause for **`full`**.
