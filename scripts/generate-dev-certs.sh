...
# gitignored.
#   - fingerprints.json is consumed by the Vite config
#     (packages/web-app/vite/fingerprints.ts) and, since story task #18, ALSO the
#     Playwright e2e global-setup (packages/web-app/e2e/global-setup.ts:25).
#   - fingerprints.env is the shell-sourceable form of the same MC/MH pair. It has
#     NO code consumer on this branch — #18's global-setup reads the .json, not this
#     file (this comment previously mis-stated .env as #18's reader) — so it exists as
#     the sourceable convenience form only; keep it as such, do not remove it.
#
...
# fingerprints.env — sourceable form (shell). No code consumer on this branch:
# task #18's Playwright global-setup reads fingerprints.json, not this file.
