...
# `fingerprints.env`), consumed by the Vite config and Playwright global-setup
# via MC_CERT_SHA256 / MH_CERT_SHA256 (R-36). Both files are gitignored.
#
...
# for the load-bearing base64 value (consumed by packages/web-app vite config +
# Playwright global-setup). The *_hex and *_expires_at fields are distinct
# debug/diagnostic data, not alternate encodings of the base64 value.
...
# fingerprints.env — sourceable form (Playwright global-setup / shell).
...
echo "     # 'apply' alone does not restart pods on a secret-only change (R-49 will automate this)"
