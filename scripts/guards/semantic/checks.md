...
**Fixture poles** (for the positive-leak fixture task). Must-fire spellings are realistic prefixed compounds, never a bare `kek`: a bare-token fixture reads green off a matcher that misses every production spelling (ADR-0036 §11; the same word-boundary reasoning is already recorded at `crates/dt-guard/src/common/pii_vocabulary.rs` and `crates/proto-gen/build.rs`, so cite it rather than restate it). State the effect per spelling and per matcher — a blanket "the matcher can't do compounds" line is false for `meeting_kek` on the word-boundary consumers, and false again for `transmit_key_bytes` on `metric_labels`:

- `meeting_kek` is itself a CATEGORY_A entry, so it *does* fire on the word-boundary consumers today; it is the realistic spelling a future rename must not be able to silently drop.
- `transmit_key_bytes` does *not* fire on the word-boundary consumers — `\btransmit_key\b` stops at the trailing `_bytes` because `_` is a word character — but *does* fire on `metric_labels`' substring pass. It is the trailing-compound blind spot made concrete.
- bare `kek` is inert where you want it (`\bkek\b` misses `meeting_kek`) and a false positive where you don't (it would fire on the metadata `kek_generation`), which is exactly why it is absent from the vocabulary and must be absent from the fixture.

<!-- fixture-poles:must-fire marker — fixture-spec anchor consumed by crates/dt-guard/tests/credential_leak_key_custody_fixtures.rs. The drift assertion requires every must-fire spelling BETWEEN the begin/end markers below to appear in some pos_ fixture. This is a fixture-spec marker, NOT a detection vocabulary and NOT a second list — the fenced text is the same prose the semantic-guard agent reads and nothing is enumerated twice. Backtick spans in THIS note are deliberately absent so an extractor cannot mistake them for must-fire spellings; only the sentence between the markers is parsed. Deleting these markers as redundant breaks the drift assertion. -->
<!-- fixture-poles:must-fire:begin -->
Must fire: a `meeting_kek: [u8; 32]` or `transmit_key_bytes: Vec<u8>` field assigned into an MC→MH `dark_tower.internal.v1` construction (item 11); `debug!(?meeting_kek)`; `info!("transmit_key_bytes={:x?}", transmit_key_bytes)`; `#[derive(Debug)]` on `struct Foo { meeting_kek: [u8; 32] }` (item 13); and — a ready-made plant — removing the `.skip_debug("dark_tower.signaling.v1.MeetingKekUpdate")` line from `crates/proto-gen/build.rs`, which item 13 already names as a finding.<!-- fixture-poles:must-fire:end --> **That last pole is exercised against a scratch copy, or an ephemeral mutation that is provably reverted with the reverted state asserted after the run — never as an in-tree edit.** Unlike every other plant here it would otherwise delete a live redaction control from a real file on the real build path, and normalising that diff as "something the fixture does" is itself the hazard. If it cannot be confined, drop it: item 13's text already covers the case, and it is not worth a control-deleting diff in the tree.

Highest-value plant — **judge the value, not the name**: a `Vec<u8>` read off the KEK field, moved through a helper and renamed `bytes` / `payload` / `material` / `blob`, then crossing the contract. It fires under no vocabulary and is what separates this check from the floor; a names-only fixture demonstrates the floor, not the check. Because it is also the most copy-pasteable working leak in the fixture, it carries a deliberate-leak banner, uses obviously synthetic values (reuse the `vec![0xC3u8; 32]` shape from `crates/proto-gen/tests/signaling_roundtrip.rs`), and is placed so it is not a compiled member of `mc-service`. **If this plant does not fire, that is a finding about the check — record it; never sand the plant down until it goes green, which is how the demonstration becomes theater.**

Must **not** fire: `debug!(kek_generation = gen, sender_id = id)` (metadata, not the key); `#[derive(Debug)]` on `struct Foo { meeting_kek: MeetingKek }` where `MeetingKek` wraps `SecretBox` (a redacting wrapper breaks the transitive reach); and the hand-rolled redacting `Debug` at `crates/proto-gen/src/lib.rs` that prints `RedactedLen(self.meeting_kek.len())` — a hand-rolled `Debug` is a finding only when it prints the bytes, and this one prints a length, so it doubles as the clean-pass anchor for "the guard passes on the real contract". The SECOND clean-pass anchor is the real MC logging sites: the run must confirm the guard stays silent there too, or it demonstrates only that the check fires, never that it is aimed at anything.

**The wrapper pole's field spelling deliberately MATCHES the must-fire pole above** — `meeting_kek` in both — so the wrapper TYPE is the only variable between them. A pole pair differing in both name and type is silently satisfied by a check that merely name-matches, having never exercised the wrapper logic at all: it fires on the must-fire because the name matches, and stays silent on the must-not-fire because the name does not. Do not "tidy" the spellings apart.

`meetingKek` and other camelCase spellings are **not** planted here, a deliberate deviation from task #23's illustrative `e.g.` list. Items 11-13 are Rust and MC-scoped and give no signal on TypeScript; MC has no camelCase serde surface, so a camelCase plant is alive-but-out-of-scope — the failure ADR-0036's demonstration section calls worse than an absent control. The one surface where `meetingKek` would be the natural spelling is MC→client delivery, and `JoinResponse.meeting_kek` and the KEK-push message are §4's delivery to an *entitled* holder, which the SAFE list above records as not leakage — so a `meetingKek` must-fire plant would contradict that carve-out, not merely sit out of idiom.

Do not restate the vocabulary in the fixture or here: `meeting_kek` / `transmit_key` live in `pii_vocabulary.rs`, and a second list drifts from the first silently.
